...
/** Cuatro indicadores al 25%: descanso queda fuera del cálculo y de la cobertura. */
export function calculateZone(code: string, districts: readonly ZoneObservation[] = ZONE_DISTRICTS): ZoneScoring | null {
  ...
  const indicators = INDICATORS.map(({ key, label, unit, inverse }): ZoneIndicator => {
    ...
    return { key, label, unit, rawValue, index, points: index == null ? null : ZONE_INDICATOR_POINTS * index, inverse,
  ...
  return { method: "zone-percentiles-v2", district: district.district, districtCode: code, scope: "distrito",
    indicators, available, coveragePercent: available * ZONE_INDICATOR_POINTS,
