...
export function personalScore(p: Property, e: PropertyEnrichment, profile: UserProfile | null): { score: number; scoring: PersonalScoring } {
  ...
  const components = [
    component("fair", "Fair · precio", weights.alpha, fair, fairScoring
      ? `Regla provisional: Fair = limitar(50 − 2,5 × desviación %, 0, 100). Desviación del anuncio frente a la estimación: ${round(fairScoring.gapPercent)}%. Coincidencia = 50; 20% por debajo = 100; 20% por encima = 0. ${p.operation === "rent" ? `Compara mensualidades; renta derivada de venta y ratios proyectados a ${e.valuation?.rentaEscenario?.ano ?? "periodo no informado"}, con últimas fuentes de venta de ${e.valuation?.proyeccion?.ultimaVentaObservada ?? "periodo no informado"} y alquiler de ${e.valuation?.proyeccion?.ultimoAlquilerObservado ?? "periodo no informado"}. Sin validación independiente de alquiler.` : `Estimación de oferta de 2018 proyectada a ${e.valuation?.nivelPrecios ?? "periodo no informado"}; última fuente de venta de ${e.valuation?.proyeccion?.ultimaVentaObservada ?? "periodo no informado"}. Precisión actual no validada.`} No expresa confianza del modelo ni una tasación.`
