...
\hypersetup{pdftitle={HabitIA: memoria y anexos},pdfauthor={Equipo 7 HabitIA}}
...
El asistente permite buscar y comparar viviendas mediante lenguaje natural. Un modelo de lenguaje de Anthropic interpreta la petición, selecciona herramientas y redacta la respuesta; las cifras proceden de las mismas consultas y cálculos que utiliza el buscador. La interfaz muestra los resultados completos de las herramientas en tarjetas, mientras el modelo recibe un resumen para explicarlos.

Ante ``busco un piso de dos habitaciones en Chamberí para comprar, hasta 500.000 €'', puede buscar anuncios, solicitar su valoración y calcular el trayecto si el perfil incluye un lugar de trabajo. El servidor ejecuta las herramientas y devuelve sus resultados al modelo, que puede continuar o responder. Cada petición admite hasta tres llamadas al modelo y cuatro ejecuciones de herramientas, con tiempos límite. Los errores se comunican como tales, sin completar las cifras ausentes.

La búsqueda se restringe a Madrid capital. Los datos del perfil y de los anuncios se tratan como datos, nunca como instrucciones; además, se validan las peticiones y se limita su frecuencia. Estas medidas reducen riesgos, pero no garantizan la fidelidad de las respuestas: la selección de herramientas y su interpretación no se han evaluado sistemáticamente. El catálogo, el flujo y los detalles de ejecución se recogen en el anexo~14.3.
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.78\linewidth]{app_inicio.png}
\par\vspace{8pt}
\includegraphics[width=0.78\linewidth]{app_ficha.png}
...
HabitIA combina un modelo de estimación de precios con una aplicación de búsqueda y comparación de vivienda en Madrid capital. Se construyó con 75.469 anuncios de 2018 enriquecidos con indicadores de barrio, separando entrenamiento y prueba para el modelado, aunque la exploración inicial utilizó el conjunto completo. XGBoost obtuvo los mejores resultados entre las familias comparadas. Su versión aplicable a anuncios actuales alcanza un error porcentual mediano del 9,3 \% en prueba, con cuatro de cada cinco viviendas dentro de un margen del 20 \%.

El precio depende principalmente de la superficie y la localización; pocas variables concentran la información predictiva. Prescindir de los datos catastrales para aplicar el modelo a anuncios actuales cuesta alrededor de un punto de error mediano, sobre todo en viviendas pequeñas y baratas. También se detectaron relaciones aparentes que requieren cautela: la asociación entre incidencias y precio reflejaba la centralidad de los barrios.
...
La evaluación con anuncios recientes, idealmente con precios de cierre, mediría la precisión por distrito, tipología y calidad de información. Se propone entrenar un modelo de alquiler, actualizar los índices con datos registrales, cuantificar la incertidumbre mediante regresión cuantílica o predicción conformal y validar con particiones espaciales o temporales.

Se propone recalibrar el HabitIA Score mediante percentiles de referencias específicas y actualizables, con 100 como posición relativa más favorable y 0 como menos favorable. Fair partiría de las desviaciones porcentuales con signo entre precio anunciado y estimado en el test, contrastadas con datos actuales. Opportunity requeriría una referencia de evolución territorial y justificar su interpretación como oportunidad. Zone ya usa percentiles; normalizaría sus indicadores cuando corresponda y revisaría la referencia territorial. Lifestyle compararía los tiempos de los candidatos que superen los filtros, antes de seleccionar resultados, y dependería de esa búsqueda, manteniendo visibles los minutos. Calibrar el Score con el test exigiría evaluarlo en otra muestra independiente; las métricas previas del predictor se mantienen.

% ============================= 14 =============================
\clearpage
\section{Bibliografía}

\begin{enumerate}[label={[\arabic*]},leftmargin=2.2em,itemsep=2pt,parsep=0pt,topsep=4pt]
\item Rey-Blanco et al. (2024). \href{https://github.com/paezha/idealista18}{\emph{idealista18}: anuncios georreferenciados de vivienda}.
\item Chen y Guestrin (2016). \href{https://doi.org/10.1145/2939672.2939785}{\emph{XGBoost: A Scalable Tree Boosting System}}.
\item XGBoost Developers. \href{https://xgboost.readthedocs.io/en/stable/parameter.html}{\emph{XGBoost Parameters}}.
\item Duan (1983). \href{https://doi.org/10.2307/2288126}{\emph{Smearing Estimate: A Nonparametric Retransformation Method}}.
\item XGBoost Developers. \href{https://xgboost.readthedocs.io/en/stable/python/python_api.html}{\emph{Python API Reference}} y \href{https://xgboost.readthedocs.io/en/stable/tutorials/saving_model.html}{\emph{Introduction to Model IO}}.
\item Perales Lara (2026). \href{https://github.com/tomasper17/house-pricing-model-habitia}{\emph{HabitIA Predictor}: código y documentación del modelo}.
\item Equipo HabitIA (2026). \emph{Verificación del modelo y su integración}. Anexos 10, 12 y 14.
\item Ayuntamiento de Madrid. \emph{SERPAVI: alquiler por sección censal}.
\item INE (2018). \href{https://www.ine.es}{\emph{Cartografía de secciones censales}}.
\item Ayuntamiento de Madrid. \href{https://datos.madrid.es}{\emph{Incidencias policiales y población por distrito y barrio}}.
\item Ayuntamiento de Madrid. \href{https://datos.madrid.es/dataset/300301-0-ranking-vulnerabilidad}{\emph{Índice de vulnerabilidad territorial}}.
\item Lundberg et al. (2020). \href{https://doi.org/10.1038/s42256-019-0138-9}{\emph{From local explanations to global understanding with explainable AI for trees}}.
\item Breiman (2001). \href{https://doi.org/10.1023/A:1010933404324}{\emph{Random Forests}}.
\end{enumerate}
Las referencias completas y las condiciones de uso de los datos figuran en el anexo~13.
% ===================== PORTADA (ANEXOS) =====================
\clearpage
...
\refstepcounter{anexosub}\needspace{3\baselineskip}%
\subsection*{\theanexo\hspace{0.3em}\arabic{anexosub}\hspace{0.45em}Referencias completas}
...
\refstepcounter{anexosub}\needspace{3\baselineskip}%
\subsection*{\theanexo\hspace{0.3em}\arabic{anexosub}\hspace{0.45em}Derechos de uso de los datos}

Los anuncios de 2018 proceden de \emph{idealista18}, de Rey-Blanco, Arbués, López y Páez, cuya base de datos se publica bajo \href{https://opendatacommons.org/licenses/odbl/1-0/}{ODbL 1.0}. Su reutilización requiere atribución y conservación de la licencia; las bases derivadas publicadas deben respetar sus condiciones de compartir igual y acceso a las transformaciones. La limpieza, selección, agregación e incorporación de indicadores se documentan en los cuadernos y el código del modelo.

Las fichas municipales de vulnerabilidad, incidencias policiales y población por distrito y barrio indican \href{https://creativecommons.org/licenses/by/4.0/deed.es}{CC BY 4.0}. Se atribuyen al Ayuntamiento de Madrid; las selecciones, agregaciones territoriales y tasas por población son elaboración propia. La cartografía de origen INE se reutiliza conforme a su \href{https://www.ine.es/ss/Satellite?L=0&c=Page&cid=1254735849170&p=1254735849170&pagename=Ayuda/INELayout}{aviso de reutilización}, con atribución, conservación de la fecha de actualización disponible y sin implicar respaldo institucional. Las tablas SERPAVI y de compraventa conservan las condiciones de su fuente original; su publicación en un portal municipal no permite atribuirles automáticamente CC BY 4.0 ni afirmar una autorización de redistribución irrestricta.

Estos permisos no se extienden a los anuncios actuales consultados mediante la API de Idealista, sujetos a las condiciones del proveedor. El ZIP de entrega incluye el código y los artefactos de inferencia, sin credenciales ni exportaciones de anuncios actuales. El archivo \texttt{DERECHOS\_DE\_USO.md} identifica las fuentes, sus enlaces y el alcance de las condiciones verificadas. Las capturas de demostración con anuncios ficticios se identifican como ejemplos sintéticos.
...
La herramienta \texttt{valorar\_vivienda} conserva los atributos y la operación original del anuncio; la referencia territorial de alquiler se presenta separada de la estimación individual.

\begingroup
\renewcommand{\thetable}{A1}
\begin{center}
\captionof{table}{Herramientas disponibles para el asistente.}
...
\end{tikzpicture}
\captionof{figure}{Flujo del asistente: la herramienta proporciona los datos de las tarjetas y el modelo de lenguaje redacta su explicación.}
\label{fig:chat}
\end{center}
\endgroup
