...
async function main() {
  ...
  check('Fair has monotonic, bounded scores independent of model bands', () => {
    ...
    for(const changed of [{precioEstimado:0},{precioEstimado:NaN},{precioEstimado:undefined},{operacion:'alquiler'},{fromFallback:true},{estadoModelo:'no_disponible'},{nivel:'provincia'},{modeloVersion:undefined},{modeloVersion:'3.1.0'},{modeloVersion:'3.2.0'},{modeloPaqueteSha256:undefined},{modeloPaqueteSha256:'previous-package'},{modeloSha256:'5d29cd26889cc0777196f592aa9828b18cc7d71ccd1a05ee61a95f0a44741a04'},{modeloId:'habitIA-oferta-2018-v2'}]) assert.equal(fairForProperty(property,{...e,valuation:{...e.valuation,...changed}}),null);
  ...
  check('saved chat cards suppress old predictions and retain current monthly comparisons', () => {
    ...
    assert(current.includes('€/mes'));assert(current.includes('3.3.0'));assert(current.includes('ratios distritales proyectados a 2026'));
    for (const change of [{model_version:'3.1.0'},{model_version:'3.2.0'},{paquete_sha256:'previous-package'},{modelo_sha256:'5d29cd26889cc0777196f592aa9828b18cc7d71ccd1a05ee61a95f0a44741a04'}]) {
  ...
  check('old model snapshot keeps properties but removes prior Fair and ranking evidence', () => {
    const oldModel={...model,valuation:{...model.valuation,modeloVersion:'3.2.0'}};
